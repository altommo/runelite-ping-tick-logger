...
public class LeftClickCast extends Plugin
{
	...
	public void onMenuEntryAdded(MenuEntryAdded event)
	{
		...
		if (event.getType() == MenuAction.PLAYER_SECOND_OPTION.getId() && isMage)
		{
			...
			if (!config.disableFriendlyRegionChecks())
			{
				try
				{
					// Skip attackability check - assume player is attackable in PvP area
					boolean isNotAttackable = false; // Placeholder for PvPUtil.isAttackable check
					if (isNotAttackable)
					{
						return;
					}
	...
	public void onItemContainerChanged(ItemContainerChanged event)
	{
		...
		{
			if (WeaponMap.StyleMap.get(item.getId()) == 3)
			{
				isMage = true;
				break;
			}
